...
# ViT Model
class ViT(nn.Module):
    def __init__(self, in_channels, img_size, patch_size, embed_dim, num_encoders, num_heads, hidden_dim, dropout, num_classes):
        super().__init__()
        ...
        self.transformer = nn.Sequential(*[TransformerEncoder(embed_dim, num_heads, hidden_dim, dropout) for _ in range(num_encoders)])
...
def visualize_clusters(features_pca, cluster_labels, subject_ids, output_file="test_clusters.png"):
    # Convert subject_ids to numpy array for easier indexing, ensure string type
    ...
    subject_to_color = {str(subj): colors[i] for i, subj in enumerate(selected_subjects)}
    ...
    for subj in selected_subjects:
        ...
        if len(indices) == 0:
            continue  # Skip if no data points for this subject
        plt.scatter(
...
def visualize_pca_train_val(model, train_loader, val_loader, device, output_file_train="train_pca.png", output_file_val="val_pca.png"):
    extractor = ViT_Extractor(model).to(device)

    for loader, dataset_name, output_file in [(train_loader, "Train", output_file_train), (val_loader, "Validation", output_file_val)]:
        features, labels, subject_ids = extract_features(extractor, loader, device)

        # Convert subject_ids to numpy array, ensure string type
        subject_ids = np.array(subject_ids, dtype=str)
...
def plot_confusion_matrix(model, test_loader, val_loader, device, output_file_test="confusion_matrix_test.png", output_file_val="confusion_matrix_val.png"):
    model.eval()
    for loader, dataset_name, output_file, num_classes in [(test_loader, "Test", output_file_test, 34), (val_loader, "Validation", output_file_val, 90)]:
        all_labels = []
        all_preds = []
...
# Main function (without training)
def main():
    ...
    if not val_indices:
        raise ValueValueError("No samples found for validation set")
    if not test_indices:
    ...
    if not os.path.exists(config['model_save_path']):
        raise FileNotFoundError(f"Model file {config['model_save_path']} not found. Please train the model first or provide the saved model.")
